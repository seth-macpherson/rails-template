RSpec.shared_context 'a resource controller' do |extra_options|
  ...
  describe 'PUT #update', unless: (extra_options || {}).fetch(:except, []).include?(:update) do
    context 'with valid params' do
      it 'updates the record' do
        params = (extra_options || {}).fetch(:shallow, true) ? {} : payload
        params.merge! Hash['id', record.to_param, resource_key, new_attributes]
    ...
    context 'with invalid params' do
      it 'does not update the record' do
        params = (extra_options || {}).fetch(:shallow, true) ? {} : payload
        params.merge! Hash['id', record.to_param, resource_key, invalid_attributes]
        put :update, params: params
        record.reload
  ...
  describe 'DELETE #destroy', unless: (extra_options || {}).fetch(:except, []).include?(:destroy) do
    it 'destroys the requested record' do
      ...
      params = (extra_options || {}).fetch(:shallow, true) ? {} : payload
      params.merge! Hash['id', record.to_param]
      expect {
